...
const {srcDir, buildDir, GPlayerPro, GPlayerDev, destDir} = config.dir;
...
// copy lib files to src directory firstly
gulp.task('copyLibToBuild', () => {
  gulp.src([
    'node_modules/dashjs/dist/dash.all.min.js',
    'node_modules/videojs-contrib-hls/dist/videojs-contrib-hls.min.js',
    'node_modules/videojs-contrib-dash/dist/videojs-dash.min.js',
    'node_modules/video.js/dist/video-js.min.css',
    'node_modules/video.js/dist/video.min.js',
    srcDir + 'swf/TanVodPlayer.swf',
    srcDir + 'swf/GgPlugin.swf'
  ])
  .pipe(gulp.dest(buildDir + 'GPlayer/lib/'))
})

// gulp.task('copy')

gulp.task('scripts', ['copyLibToBuild'], ()=> {
  return browserify({
    entries: './build.js',
    debug: true,
    transform: [babelify]
  ...
  .bundle()
  .pipe(source('app.js'))
  .pipe(gulp.dest(buildDir + 'GPlayer/'))
})
